...
基準は Android アダプティブアイコンの 108dp レイヤー内に取る**参照円 Ø70dp（= 0.648）**。
コンテンツのバウンディングボックス対角がこの円に内接する倍率を算出し、iOS・Android・favicon・Web ホームアイコンすべてに同じ倍率を適用しています。背景は各アイコンとも全面のままで、中央のロゴのみ拡縮しています。

BWU ワードマークの原寸は 3 デザインとも同一（キャンバス比 0.669）なので、**同じ倍率＝同じ見た目サイズ**になります。

| デザイン | 倍率 | BWU ロゴ幅 | 実測半径（432px） |
|---|---|---|---|
| user-app | ×0.9266 | **0.6199** | 139.4 px |
| web-home-icon | ×0.9266 | **0.6199** | 139.4 px |
| manager-cms | ×0.9034 | **0.6043** | 144.0 px |

### 参照円を Ø70dp にしている理由

Android の規定値は「**保証セーフ円 Ø66dp**」と「**可視ビューポート 72dp**」の 2 つです。Ø66dp はランチャーのマスク形状やアニメーションを問わず必ず見える範囲、Ø72dp は円マスクの実際の可視域＝絶対に超えられない上限です。

Ø66dp 基準だとロゴが小さく見えたため、**Ø70dp を採用**しています（Ø66 比 +6.1%）。

### manager-cms だけ倍率が異なる理由

manager-cms は「Manager」の文字を含むぶんコンテンツブロックが縦に高く、user-app と同じ ×0.9266 にすると外接円の半径が 147.7px となり、**Ø72dp ビューポート（半径 144.0px）を 3.7px はみ出して円マスクで角が欠けます**。

そのため BWU ロゴ幅を揃えることを優先しつつ、**円マスクの限界いっぱい（半径 144.0px ちょうど）まで寄せた ×0.9034** としています。BWU ロゴ幅の差は user-app 比 **−2.5%** で、実用上は判別できません。

なお外接円の判定には bbox 対角ではなく**実測の最遠画素距離**を使っています。manager-cms は下段の「Manager」が上段の BWU より狭く、bbox 対角では過大評価になるためです。

## 背景グラデーション（user-app）

Figma 原本はグラデーションの終端ハンドルが対角 68.8% の位置にあり、**右下 31.2% がフラットな純黒**になって段差が見える状態でした。終端をキャンバス右下隅に合わせ、`#656565`（左上）→ `#000000`（右下隅）の全面連続ランプに作り直しています。`favicon.svg` の `linearGradient` も同じ座標に揃えてあります。
...
前景レイヤーのロゴは円マスクの可視域 **Ø72dp の内側**に収まっているため、どのランチャーのマスク形状（円・角丸四角・スクワークル）でも欠けません。
