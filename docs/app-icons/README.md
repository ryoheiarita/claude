...
| デザイン | 倍率 | ロゴ幅（キャンバス比） |
|---|---|---|
| user-app | ×0.9266 | 0.670 → **0.621** |
| web-home-icon | ×0.9266 | 0.670 → **0.621** |
| manager-cms | ×0.8510 | 0.670 → **0.570** |

manager-cms のみ倍率が小さいのは、コンテンツに「Manager」の文字が加わり縦に高い＝対角が長いためです（BWU ロゴ単体ではなくブロック全体を参照円に収めています）。

### 参照円を Ø70dp にしている理由

Android の規定値は「**保証セーフ円 Ø66dp**」と「**可視ビューポート 72dp**」の 2 つです。Ø66dp はランチャーのマスク形状やアニメーションを問わず必ず見える範囲、Ø72dp は円マスクの実際の可視域です。

Ø66dp 基準だとロゴが小さく見えたため、**Ø70dp を採用**しています（Ø66 比 +6.1%）。円マスクの可視域 Ø72dp に対して 2dp のマージンが残るため、実用上どのランチャーでも欠けません。

実測値（432px キャンバス）：前景レイヤーのコンテンツ最遠画素は user-app **139.2px** / manager-cms **136.2px**。Ø72dp ビューポート半径 144.0px の内側に収まっています（Ø66dp 保証円の半径は 132.0px）。Web マスカブルは 165.2px（セーフ円半径 204.8px、512px キャンバス）で十分な余裕があります。
...
前景レイヤーのロゴは **Ø70dp 参照円に内接するサイズ**。円マスクの可視域 Ø72dp の内側に収まっているため、どのランチャーのマスク形状（円・角丸四角・スクワークル）でも欠けません。
