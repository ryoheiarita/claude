...
| ファイル | タグ / 種別 | 中身 | 使用画面 |
| --- | --- | --- | --- |
| `bwu-action-rail.js` | `<bwu-action-rail>` | 縦動画の**右側 縦並びアクションボタン**（♥/🔖/💬/📄概要 の**4つ**） | 縦動画・縦LIVE |
...
| アイコン（見かけ） | **`18×18` 相当に統一**（線 white）。素材ごとの実測値・スケールは **[アクションレール実装仕様](../rail/)** 参照 | 静的SVGフォールバックのみ `24×24` 枠・`stroke 1.6` |
...
- **60fps / 全 164 フレーム**。フレーム: `LIKE_START=0`、`FILLED=73`（押下＝0→73 で停止）、`UNLIKE_START=92`（解除＝92→終端(164)→0）。
- ⚠ **2026-08-01 に素材データを再タイム**：冒頭にあった無動作 17 フレーム（283ms）を削除し、**動き出し＝frame 0** にした（タップしてから動くまでの間を無くすため）。旧データの `17/90/105/181` は使わないこと。**必ず同梱の新しい JSON を使う**（旧 JSON ＋ 新定数、または新 JSON ＋ 旧定数はどちらも破綻する）。
- **連打ガード**（アニメ中は無視）。
...
> **正仕様は [アクションレール実装仕様（実寸）](../rail/) を参照。**
> 縦動画に現在実装されている状態が正で、実寸カンプ・アイコンの実測値・配置数値はそちらに集約しています。以下は要約。

縦画面の右側に**縦並び**で並ぶ **4 ボタン**。縦動画での指定は `buttons="heart,bookmark,comment,detail"`。

- 各 `48×48` 円（border 込みの実寸＝`box-sizing: border-box`）、`gap 8` → **群の高さ 216**。配置はホスト側（`position:absolute; right:16px; top:…`）。
- **♥/🔖 = トグル**（§2 の Lottie、コンポーネント内で完結）。
- **💬/📄 = ワンショット**（タップで Lottie を頭から1回再生し、イベント通知のみ。シート開閉はホスト側）。
- **⛶ 全画面ボタンは無し**（2026-07-04 変更）。以前の 5 ボタン構成は廃止し、全画面はプレイヤー側コントローラーに一本化。コンポーネントに `fullscreen` の実装は残るが**縦動画では使わない**。
- **アイコンの見かけは 18×18 相当で統一**（♥🔖 は素材原寸のまま／💬📄 は読み込み後に長辺18・中央へ正規化）。Flutter は素材ごとに固定スケールを焼き込めばよい（値は [rail 仕様](../rail/) の表）。

**配置（縦動画）**:
| 状態 | 位置 |
| --- | --- |
| 通常 | `right:16` ／ **ボタン群の下端＝動画下端の 48px 上**（動画が縦中央寄せなのでそれに追従。`top = 動画top + 動画h − 216 − 48`） |
| コメント開 / 概要開 | `right:16` ／ `top = safe-area-top + 16`（戻るボタンと上揃え） |

遷移は `top`/`right` を **0.42s `cubic-bezier(0.22,0.61,0.36,1)`**、重なり順は動画・プレイヤーコントロールより上／シートより下（`z-index 55`）。
...
- 属性 `buttons="heart,bookmark,comment,detail"` … 表示ボタンの選択・並べ替え（＝縦動画の指定）。
- 属性 `comment-artists="url1,url2,url3"` … 起動時にアーティスト巡回を開始（最大3）。

### 3.1 コメントボタン：アーティストコメント時のアイコン巡回（2026-07-03 追加）

> **動くデモ付きの正仕様は [rail 実装仕様 §6](../rail/#artist)**（アーティスト有り／無しを並べて確認できる）。

そのコンテンツに**アーティスト本人のコメントが付いている場合だけ**、コメントボタンの中身が
吹き出しアイコン⇄アーティストのアバターに「ポワン」と切り替わりながら巡回する。
「アーティストが喋っている」ことをレール上で気づかせるための表現。
...
**2026-08-01**
- **タップ後の「もっさり」を解消：全アイコンとも素材データの頭を詰めて、動き出し＝frame 0 に統一**（実装側での開始位置補正は不要）。
  - ♥🔖（`lottie-data.js` / 単体 JSON）: 頭 17f = 283ms 削除、全 181f → **164f**。定数は `LIKE_START=0 / FILLED=73 / UNLIKE_START=92`。**解除の 67ms 遅れも解消**。
  - 💬📄⛶（`rail-icons.js`）: 💬 5.994f=**200ms** / 📄 4.795f=**160ms** / ⛶ 14.385f=480ms を削除。全長は 53f / 54f / 45f（29.97fps）。
  - いずれも**見た目は旧データと全フレーム一致**（旧 `f + 削った頭` ＝ 新 `f` で全フレーム描画比較して検証）。
  - `bwu-action-rail.js` には「最初に動き出すフレーム」を自動計測する保険（tabbar-260726 と同一手法の `firstMotionFrame()`）を残してあるが、再タイム済み素材では 0 を返すので実質何もしない。
- **rail の正仕様を [アクションレール実装仕様（実寸）](../rail/) に集約**。縦動画に実装されている状態が正。
- 💬📄 のアクティブ（白枠）は**排他・トグル**（片方を点けるともう片方は消灯／アクティブを再タップで解除）と明記。
- rail は **4 ボタン**（♥/🔖/💬/📄）で確定。**⛶ 全画面ボタンは廃止**（プレイヤー側コントローラーに一本化）。群の高さ **216**。
- アイコンの**見かけサイズを 18×18 相当に統一**（💬📄 は長辺18・中央へ正規化。Flutter は固定スケールで焼き込み）。
